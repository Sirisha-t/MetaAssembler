...
public:
  ...
  void processNodeHeader(const boost::tokenizer<boost::char_separator<char>>& tokens);
  void processEdgeHeader(
      const boost::tokenizer<boost::char_separator<char>>& tokens,
      const std::string& header, std::string::size_type colon
  );
  void processEdgeRest(boost::tokenizer<boost::char_separator<char>>::iterator it, boost::tokenizer<boost::char_separator<char>>::iterator end, const BoostSTRVertex& v_source, std::vector<ReadInfo>& read_info);
  };
#endif
